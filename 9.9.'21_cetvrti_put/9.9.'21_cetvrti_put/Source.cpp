...
public:
	...
	friend  ostream& operator<<(ostream& COUT, const Tehnika& obj) {
		...
		{
			COUT << " Ocjena--->" << obj._ocjene->getElement1(i) << " - " << *obj._ocjene->getElement2(i) << endl;
		}
...
bool ValidirajLozinku(string lozinka) {
	return regex_search(lozinka, regex("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^a-zA-Z0-9]).{7,}$"));
}
...
public:
	...
	void PosaljiMail(Pojas pojas, Tehnika tehnika, float prosjekTehnike) {
		...
		cout << "Postovani" << GetImePrezime() << "evidentirana vam je thenika" << tehnika << "za" << IspisPojasa[pojas] << "pojas" << "Dosadasnji uspjeh (prosjek ocjena)" << endl;
		cout << "na pojasu" << IspisPojasa[pojas] << "ïznosi" << prosjekTehnike << "a ukupni uspjeh (prosjek ocjena) na svim pojasevima iznosi" << UkupniProsjekPojasa() << endl;
		mu.unlock();
	}
	bool AddTehniku(Pojas pojas, Tehnika tehnika) {
		for (int i = 0; i < _polozeniPojasevi.size(); i++)
		{
			...
			thread t1(&KaratePolaznik::PosaljiMail, this, pojas, tehnika, _polozeniPojasevi[i].ProsjekPojasa());
			t1.join();
			return true;
		}
		for (int i = 0; i < _polozeniPojasevi.size(); i++)
		{
			if (pojas > _polozeniPojasevi[i].GetPojas() && (_polozeniPojasevi[i].GetTehnike().size() < 3 || _polozeniPojasevi[i].ProsjekPojasa() < 3.5))
				return false;
			_polozeniPojasevi[i].GetTehnike().push_back(new Tehnika(tehnika));
			thread t2(&KaratePolaznik::PosaljiMail, this, pojas, tehnika, _polozeniPojasevi[i].ProsjekPojasa());
			t2.join();
			return true;
		}
		Polaganje polaganje;
		polaganje.GetTehnike().push_back(new Tehnika(tehnika));
		thread t3(&KaratePolaznik::PosaljiMail, this, pojas, tehnika, polaganje.ProsjekPojasa());
