...
unique_ptr<char[]> GetnNizUnique(const char* tekst) {
    if (tekst == nullptr)
        return nullptr;
    int vel = strlen(tekst) + 1;
...
public:
    ...
    void  AddOcjenuKriterija(Kriteriji krit, int ocjena) {
        if (ocjena > 10 && ocjena < 0)
            throw exception(" Ocjena je izvan dosega");
        for (int i = 0; i < _ocjeneKriterija->getTrenutno(); i++)
        {
            if (krit == _ocjeneKriterija->getElement1(i))
                throw exception(" kriterij je vec komentarisan");

        }
        _ocjeneKriterija->AddElement(krit, ocjena);

    }
    float prosjcnaKomentar()const {
        float prosjek = 0.0f;
        for (int i = 0; i < _ocjeneKriterija->getTrenutno(); i++)
        {
    ...
    friend ostream& operator<<(ostream& COUT, const Komentar& obj) {
        ...
        COUT << " prosjecna ocjena je: "<<obj.prosjcnaKomentar() << endl;
...
public:
    Gost(const char* imePrezime, string emailAdresa, string brojPasosa) {
         _imePrezime = GetnNizUnique(imePrezime);
        _emailAdresa = emailAdresa;
        _brojPasosa = ValidirajBrojPasosa(brojPasosa) ? brojPasosa : "NOT_VALID";
    }
    Gost(const Gost& obj) {
        _imePrezime = GetnNizUnique(obj._imePrezime.get());
...
class Rezervacija {
    ...
    Komentar _komentar;
    void saljiEmail(float prvi) {
        mu.lock();
        cout << " To: denis@fit.ba;jasmin@fit.ba;\} Subject: Informacija\nPostovani,\n\n  Zaprimili smo Vase ocjene, a njihova prosjecna vrijednost je " << prvi << " Zao nam je zbog toga, te ce Vas u najkracem periodu kontaktirati nasa Sluzba za odnose sa gostima.\nUgodan boravak Vam zelimo\nPuno pozdrava" << endl;
...
public:
    ...
    void SetKomentar(Komentar kom) {
        _komentar = kom;
        //ukoliko se rezervaciji doda komentar koji sadrzi najmanje dvije ocjene kriterija sa vrijednoscu manjom od 5, svim gostima, 
        
        int brojac = 0;
           /* if(kom.GetOcjeneKriterija()->getTrenutno()>2 )
                if (kom.prosjcnaKomentar() > 4) {
                    thread t1(&Rezervacija::saljiEmail,this, kom.prosjcnaKomentar());
                    t1.join();
                }*/
        for ( int i = 0; i < kom.GetOcjeneKriterija()->getTrenutno(); i++)
        {
            if (kom.GetOcjeneKriterija()->getElement2(i) < 5)
                brojac++;
