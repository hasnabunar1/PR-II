...
const char* IsisPojasa[] = { "BIJELI","ZUTI","NARANDAZSTI","ZELENI"," PLAVI","SMEDJI","CRNI" };
...
bool ValidirajLozinku(string lozinka) {
	return regex_search(lozinka, regex("[A-Z]")) && regex_search(lozinka, regex("[a-z0-9]")) && regex_search(lozinka, regex("[^A-Za-z0-9]")) && regex_search(lozinka, regex(".{7,}"));
 }
...
public:
	...
	Kolekcija<T1, T2> /*void*/  RemoveAt(int index) {
		...
		(*_trenutno)--;
		nova.AddElement(*_elementi1[*_trenutno], *_elementi2[*_trenutno]);
		return nova;
	...
	void AddElement(T1 el1, T2 el2, int lokacija) {
		...
		_elementi1[lokacija - 1] = new T1(el1);
		_elementi2[lokacija - 1] = new  T2(el2);
		//	cout << "trenutno" << *_trenutno << endl;
...
public:
	...
	*/
		if (ocjena > 5 && ocjena < 1)
			throw exception(" ocjean je izvan opsega!");
		...
		for (int i = 0; i < trenutno; i++)
		{
			...
			if (abs(_ocjene->getElement2(i)->ToDays() - datum.ToDays()) < 3)
	...
	friend  ostream& operator<<(ostream& COUT, const Tehnika& obj) {
		...
		COUT << "prosjecna ocjena u tehnici " << obj.ProsjekUTehnici() << endl;
		return COUT;
	 }
};
...
public:
	...
	float	ProsjekUPolaganju()const {
		float prosjek = 0.0f;
		for (int i = 0; i < _polozeneTehnike.size(); i++)
		{
			for (int j = 0; j < _polozeneTehnike[i]->GetOcjene().getTrenutno(); j++)
			{
				prosjek += _polozeneTehnike[i]->GetOcjene().getElement1(j);
			}
	...
	friend ostream& operator<< (ostream& COUT, const Polaganje& obj) {
		...
		COUT << " prosjek svih tehnika " << obj.ProsjekUPolaganju() << endl;
		return COUT;
	}
...
class KaratePolaznik :public Korisnik {
	...
	void SaljiMail(char* naziv,Pojas pojas,float prosjek,float ukupni) {
		mu.lock();
		cout << "FROM:info@karate.ba\nTO:" << GetEmail() << " \nPostovani " << GetImePrezime() << ",\n evidentirana vam je tehnika " << naziv << "  za " << IsisPojasa[pojas] << " pojas.Dosadasnji uspjeh(prosjek ocjena) na pojasu  " << IsisPojasa[pojas] << " iznosi " << prosjek << " , a ukupni uspjeh(prosjek ocjena) na svim pojasevima iznosi " << ukupni << ".\nPozdrav.\nKARATE Team." << endl;
		mu.unlock();
		...
		if (pojas > ZUTI) {
			for (int i = 0; i < _polozeniPojasevi.size(); i++)
			{
				if (pojas == _polozeniPojasevi[i].GetPojas())
					...
					{
						if (_polozeniPojasevi[i].GetTehnike()[j]->GetOcjene().getTrenutno() < 3 || _polozeniPojasevi[i].ProsjekUPolaganju() < 3.5)
							return false;
					}
			...
			char* naziv = GetNizKaraktera(tehnika.GetNaziv());
			float prosjek = tehnika.ProsjekUTehnici();
			float ukupni = p.ProsjekUPolaganju();

			thread t1(&KaratePolaznik::SaljiMail, this, naziv, pojas, prosjek, ukupni);
			t1.join();
			return true;
